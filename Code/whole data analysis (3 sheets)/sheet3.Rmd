...
data <- data.frame(
  ...
  AD_negative = c(46, 9, 37, 0, 46),
  AD_positive = c(47, 30, 17, 47, 0),
  APOE4_negative = c(54, 0, 54, 17, 37),
  APOE4_positive = c(39, 39, 0, 30, 9),
  Total = c(93, 39, 54, 47, 46)
)
...
In this dataset, the **Amyloid Status** is classified based on the CSF Ab42/Ab40 ratio.
...
2. **FTR:** Fractiona; turnover rate.
3. **Delay times:** Time taken for a molecule to traverse a certain path or process.

**Constructed Variables:**

1. **FCR (Fractiona; Clearance Rate):** Calculated as \( \text{FCR} = \frac{1}{\text{delay time} + \frac{1}{\text{FTR}}} \).
...
1. **Complete Data:** 67 out of 93 individuals have complete data with no missing values.
   
2. **Specific Missing Patterns:** 

   - **Hippocampal_Vol and VentriCSF_Vol Missing:** This is the most common missing pattern, seen in 19 out of 93 individuals.
   - **Race Missing:** Observed in 2 out of 93 individuals.
   - **Both Race, Hippocampal_Vol Missing, and VentriCSF_Vol:** Also seen in 2 out of 93 individuals.
...
**Conclusion:**
If the analysis require the inclusion of the `Hippocampal_Vol` variable, multiple imputation will be necessary given its substantial missingness which cannot be overlooked.
...
df_AD %>%
  ...
  kable_styling(full_width = F, position = "left",
                latex_options = c("striped", "repeat_header"))
```
The observed imbalance in APOE status could impact the statistical validity of subsequent findings, warranting caution during interpretation.

For future investigations, employing oversampling techniques might mitigate this issue.
...
- Comprehensive visualization outputs are relegated to the "Visualization-Results-Sheet3.pdf".
...
Before p-value correction for multiple tests, there are two significant models: FTR_ratio_sAPPb_sAPPa(p<0.1), dlt_ratio_sAPPb_sAPPa(p<0.01)
```{r}
...
See the "Visualization-Results-Sheet3.pdf" for comprehensive visualizations
...
# Specify the selected variables
selected_vars <- c("pdr_ratio_sAPPb_totalAb", "FCR_sAPPb")

# Loop through the selected variables and plot
for (var_name in selected_vars) {
  p1 <- plot_histograms(var_name, df=df_noAD, title="AD[-]: E4+ vs E4- ")
  p2 <- plot_kdes(var_name, df=df_noAD, title="AD[-]: E4+ vs E4- ")
...
# Loop through the selected variables and plot
for (var_name in selected_vars) {
  p <- plot_histograms_with_kdes(var_name, df=df_noAD, title="AD[-]: E4+ vs E4- ")
  print(p)  # If running in an interactive R session, this will display the plot.
}
...
Before adjusting for multiple testing, ten models were identified as significant: `cct_ratio_sAPPb_Ab40`, `cct_ratio_sAPPb_totalAb`, `pdr_ratio_sAPPb_totalAb`, `pdr_ratio_sAPPb_Ab40`, `FTR_sAPPa`, `FTR_sAPPb`, `FCR_sAPPa`, `FCR_sAPPb`, `FCR_ratio_sAPPb_Ab40`, and `dlt_ratio_sAPPb_Ab40`.
```{r}
...
All the coefficients except for `dlt_ratio_sAPPb_Ab40`are positive, indicating that among individuals without Alzheimer's Disease (AD[-]), those with the APOE4 gene typically have a lower level of delay times of sAPPb/Ab40, but exhibit higher levels of other sAPPb related values.

However, it's essential to note that only 9 out of 46 subjects in the AD[-] subset have the APOE4 gene, while the remaining 37 do not. Consequently, these estimates may not be entirely reliable.
...
for (i in seq_along(adjusted_p_values)) {
  if (adjusted_p_values[i] <= 0.1) {
    ...
    # Store the model in significant_models
    significant_models[[idp_names[i]]] <- model
    sig_names <- c(sig_names, idp_names[i])
  }
}
...
After correctons, there are still 5 significant models: `pdr_ratio_sAPPb_totalAb`, `pdr_ratio_sAPPb_Ab40`, `FTR_sAPPa`, `FTR_sAPPb`, and `FCR_sAPPb`.
```{r}
sig_names <- sig_names[sig_names != "pdr_ratio_Ab_sAPPb"]
```
...
for (idp in sig_names) {
  ...
  coefs <- c(coefs, coef_summary[2,1])
  stds <- c(stds, coef_summary[2, 2])
  adjusted_p_values <- c(p_values, coef_summary[2, 4])
...
The application of the correction won't alter the estimated coefficients. It suggests that among individuals without Alzheimer's Disease (AD[-]), those carrying the APOE4 allele are more likely to exhibit elevated levels of `pdr_ratio_sAPPb_totalAb`, `pdr_ratio_sAPPb_Ab40`, `FTR_sAPPa`, `FTR_sAPPb`, and `FCR_sAPPb`. 


**Normality Test**

The diagnostic normality test revealed that `pdr_ratio_sAPPb_totalAb`, `pdr_ratio_sAPPb_Ab40`, and `FTR_sAPPa` do not conform to the normality assumption. This finding also implies that the previously conducted univariate linear regression, which assumes normal distribution of the data, is not suitable for these specific variables. Consequently, we should use U-tests rather than t-tests for these variables. 
```{r}
...
for(var in adjusted_p_values_df$variable_name) {
    # Ensure that the variable exists in df_noAD
    if(var %in% names(df_noAD)) {
        # Perform the Shapiro-Wilk test
        test_result <- shapiro.test(df_noAD[[var]])
...
```
After applying suitable tests to each variable and doing multiple testing corrections, the results indicate that our previous conclusion still holds, i.e., there exist significant differences in the level of `pdr_ratio_sAPPb_totalAb`, `pdr_ratio_sAPPb_Ab40`, `FTR_sAPPa`, `FTR_sAPPb`, and `FCR_sAPPb` between individuals with APOE4 and those without in the non-patient subgroup

Note: The "small sample size plus imbalance in group sizes" problem still exists. 
...
# Loop through each variable in idp_names
for (idp in adjusted_p_values_df$variable_name) {
  
  # Perform the Shapiro-Wilk test for normality
  shapiro_test <- shapiro.test(df_noAD[[idp]])
  
  if (shapiro_test$p.value < 0.05) {
    # If the data is not normally distributed, use Wilcoxon test
    test_result <- wilcox.test(as.formula(paste(idp, "~ ApoE_Status")), data = df_noAD)
  } else {
    # If the data is normally distributed, use t-test
    test_result <- t.test(as.formula(paste(idp, "~ ApoE_Status")), data = df_noAD)
  }
...
### Control Variables Imbalance Check 

1. Mean Difference:

BMI, CDR_SOB and Hippocampal_Vol are the three imbalanced control variables according to mean difference between E4+ and E4-:
```{r}
# Assume you've created balance_table using bal.tab:
AD_control_names<-c("Age", "BMI", "CDR", "CDR_SOB", "Hippocampal_Vol","SexCode", "Race", "Cognitive_Impairment_Binary_Score")
formula <- as.formula(paste("ApoE_Status", "~", paste(AD_control_names, collapse = " + ")))
balance_table<-bal.tab(formula, data = df_AD, disp = c("mean", "sd"), thresholds = 0.1)
data <- as.data.frame(balance_table$Balance[, 1:7] %>%
  filter(abs(Diff.Un)>0.1) %>%
  arrange(desc(abs(Diff.Un))))
...
See the "Visualization-Results-Sheet3.pdf"
```{r}
####
```
...
There is one significant model: `FCR_sAPPb ~ Amyloid_Status`, at level of 0.1.
...
Among individuals with the APOE4 gene, those with Alzheimer's Disease (AD) tend to exhibit lower levels of FCR_sAPPb compared to healthy individuals.
...
for (i in seq_along(adjusted_p_values)) {
  if (adjusted_p_values[i] <= 0.1) {
    ...
    # Build the formula for lm
    formula <- as.formula(paste(idp_names[i], "~ Amyloid_Status"))
...
adjusted_p_values_df %>%
  ...
  kable_styling(full_width = F, position = "left",
                latex_options = c("striped", "repeat_header"))
```
There is no significant model anymore.
...
# write.csv(high_corr_vars, "correlation_pairs_sheet3.csv", row.names = FALSE) #threshold=0
```
...
```{r, include=FALSE, echo=FALSE}
## single test for collinearity
model_name <- "2_10"
significant_configurations[[model_name]]
...
model2 <- glm(formula = formula, family = binomial, data = df)
anova(model1, model2)
#vif(model)
```

Out of the 41 significant models we identified, many share identical combinations of independent variables, differing only in their control variable combinations. Moreover, upon examining the estimated coefficients across models, they don't exhibit substantial differences. 

Among these, only **four** distinct variables exhibited statistical significance: `FTR_ratio_sAPPb_sAPPa`, `FCR_ratio_sAPPb_sAPPa`, `cct_sAPPb`, and `dlt_ratio_sAPPb_sAPPa`. 

Of these, `FTR_ratio_sAPPb_sAPPa` and `FCR_ratio_sAPPb_sAPPa` stand out, marking their significance in 18(>=12) out of the models, whereas `cct_sAPPb` was significant in 5 models, and `dlt_ratio_sAPPb_sAPPa` in just one. This frequency suggests that `FTR_ratio_sAPPb_sAPPa` and `FCR_ratio_sAPPb_sAPPa` are robustly significant and less likely to be mere statistical artifacts. On the other hand, the significance of `dlt_ratio_sAPPb_sAPPa` might be more coincidental.
...
```{r, results='asis'}
coef_summary <- summary(significant_models[[1]])
# Rename the column
colnames(coef_summary)[colnames(coef_summary) == "term"] <- "Model 1"
coef_summary %>%
  kbl(booktabs=TRUE, longtable = TRUE, linesep="") %>%
  kable_styling(full_width = F, position = "left",
                latex_options = c("striped", "repeat_header"))
...
coef_summary <- summary(significant_models[[17]])
# Rename the column
colnames(coef_summary)[colnames(coef_summary) == "term"] <- "Model 17"
coef_summary %>% 
      kbl(booktabs=TRUE, longtable = TRUE, linesep="") %>% 
      kable_styling(full_width = F, position = "left",
                latex_options = c("striped", "repeat_header"))
...
Given the high correlation between `FCR_ratio_sAPPb_sAPPa` and `FTR_ratio_sAPPb_sAPPa`, we'll focus on interpreting the latter, the other one is similar:

- FTR_ratio_sAPPb_sAPPa:

The estimated coefficients for `FTR_ratio_sAPPb_sAPPa` are consistently **negative**. Using the first table (Model 1) as an example, a one-unit increase in `FTR_ratio_sAPPb_sAPPa` **decreases** the odds of `Amyloid_Status` being 1 by a factor of $e^{-7.12} \approx 0.0008$. This suggests that higher levels of `FTR_ratio_sAPPb_sAPPa` are associated with a lower likelihood of AD[+]. 

However, bearing in mind that the actual range for `FTR_ratio_sAPPb_sAPPa` is 0.721 to 1.109, such a one-unit increase is theoretical. In a realistic increment of 0.1 units, the odds would decrease by \(e^{-0.712}\approx 0.5\) times.

- cct_sAPPb:

For `cct_sAPPb`, the estimated coefficients are **positive**. As per the second table (Model 17), a one unit rise in `cct_sAPPb` **increases** the odds of `Amyloid_Status` being 1 by a factor of $e^{0.148} \approx 1.16$. This indicates that increased levels of `cct_sAPPb` are associated with a increased probability of AD[+].
\

In summary, the three variables, `FTR_ratio_sAPPb_sAPPa`, `FCR_ratio_sAPPb_sAPPa`, and `cct_sAPPb`, might serve as notable predictors for determining AD status.
...
To delve into this robustness, we employed the default multiple imputation method. Impressively, **all 41 model configurations** maintained their significance with this imputation approach, and the estimates were largely consistent. However, a noteworthy point is the performance of `dlt_ratio_sAPPb_sAPPa` that it shows significance in 5 models. However, due to its behavior in the PMM imputed data, we remain cautious about whether its significance might have emerged purely by chance.

For a detailed view of the results, refer to Appendix-Significant Multivariate Logistics Models (Default).

In conclusion, our findings before demonstrate a commendable robustness against the nuances of imputations.
