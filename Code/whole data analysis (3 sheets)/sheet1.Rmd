...
data <- data.frame(
  ...
  AD_negative = c(47, 7, 40, 0, 47),
  AD_positive = c(46, 32, 14, 46, 0),
  APOE4_negative = c(54, 0, 54, 14, 40),
  APOE4_positive = c(39, 39, 0, 32, 7),
  Total = c(93, 39, 54, 46, 47)
)
...
In this dataset, the **Amyloid Status** is primarily determined using the PET PiB value. If the PET PiB value is not available, the CSF Ab42/Ab40 ratio is utilized.
...
- Comprehensive visualization outputs are relegated to the "Visualization-Results-Sheet1.pdf".
...
plot_marginal_histograms <- function(var_name, df, title) {
  p <- ggplot(df, aes_string(x = var_name, y = "ApoE_Status")) +
    geom_point(aes(color = factor(ApoE_Status)), alpha = 0.5) + 
    labs(x = var_name, y = "ApoE Status", color = "ApoE Status") +
    theme_minimal() +
    ggtitle(paste(title, "(", var_name, ")")) 
  
  p_marginal <- ggMarginal(p, type = "histogram", fill = "blue")
...
# Loop through the selected variables and plot
for (var_name in selected_vars) {
  plot_marginal_histograms(var_name, df=df_AD, title="AD[+]: E4+ vs E4- ")
}
...
```
```{r}
stargazer(significant_models[[2]], type = "text")
```
...
adjusted_p_values_df %>%
  ...
  kable_styling(full_width = F, position = "left",
                latex_options = c("striped", "repeat_header"))
```
After correction, the Delay times sAPPb/sAPPa variable still presents a borderline adjusted p-value of 0.057(close to 0.05 < 0.1), this result holds even under the "conservative" Bonferroni correction. 

The correction process only adjusts the p-value without altering the coefficient estimate for ApoE_Status, which remains at -0.034.


**Regression Interpretation**

At a significance level of 0.01, it's observed that on average, AD patients with APOE4 have a 0.034 lower ratio of `Delay Times sAPPb/ sAPPa` compared to those without APOE4. This suggests that there is also a higher level of `Delay Times sAPPb/ sAPPa` in AD patients without APOE4. The attached distribution plot illustrates this, where the red line representing patients without APOE4 is further to the right than the green one (for APOE4[+]).

**Normality Test**

For diagnostic purposes, the Shapiro-Wilk test was employed to assess the normality of the variable `dlt_ratio_sAPPb_sAPPa` within the specified subset. The test yielded a p-value greater than 0.05 (p-value = 1), indicating no significant deviation from normal distribution. Consequently, this supports the assumption of normality for `dlt_ratio_sAPPb_sAPPa`, enhancing the reliability of our results.

```{r}
shapiro.test(df_AD$dlt_ratio_sAPPb_sAPPa)
#t.test(df_AD$dlt_ratio_sAPPb_sAPPa ~ df_AD$ApoE_Status)
```
...
BMI, CDR and SexCode are the three imbalanced control variables according to mean difference between E4+ and E4-:
```{r}
# Assume you've created balance_table using bal.tab:
AD_control_names<-c("Age", "BMI", "CDR", "CDR_SOB", "Hippocampal_Vol","SexCode", "Race", "Cognitive_Impairment_Binary_Score")
df_AD <- df_AD %>%
  na.omit(AD_control_names)
...
See "Visualization-Results-Sheet1.pdf" AD[+] control variable part.
...
See "Visualization-Results-Sheet1.pdf"
...
Prior to the correction for multiple testing, seven models were identified as significant: `pdr_ratio_sAPPb_totalAb`, `pdr_ratio_sAPPb_Ab40`, `FTR_sAPPa`, `FTR_sAPPb`, `FCR_sAPPa`, `FCR_sAPPb`, and `dlt_ratio_sAPPb_Ab40`.
...
However, it's essential to note that only 7 out of 47 subjects in the AD[-] subset have the APOE4 gene, while the remaining 40 do not. Consequently, these estimates may not be entirely reliable.
...
adjusted_p_values_df %>%
  ...
  kable_styling(full_width = F, position = "left",
                latex_options = c("striped", "repeat_header"))
```
Following correction, none of the models exhibit significance concerning ApoE_Status.
...
See the "Visualization-Results-Sheet1.pdf"
```{r}
####
```
...
lapply(2:length(my_list), function(i) write.csv(my_list[[i]],
 file = paste0('./data/sheet1_imputed/imputed_', names(my_list[i]), ".csv"),row.names = FALSE))
```
...
# write.csv(high_corr_vars, "correlation_pairs_sheet1.csv", row.names = FALSE) #threshold=0
...
Among the 14 significant models identified, we present a detailed exploration of two as representative examples. For the complete set of results of significant models, please refer to the Appendix-Significant Multivariate Logistics Models (PMM).
...
coef_summary <- summary(significant_models[[9]])
# Rename the column
colnames(coef_summary)[colnames(coef_summary) == "term"] <- "Model 9"
coef_summary %>% 
      kbl(booktabs=TRUE, longtable = TRUE, linesep="") %>% 
      kable_styling(full_width = F, position = "left",
                latex_options = c("striped", "repeat_header"))
...
Despite having 14 significant models, several of them share the same independent variable combinations but differ in their control variable combinations. Moreover, upon examining the estimated coefficients across models, they don't exhibit substantial differences. 

Notably, only **three** variables consistently show statistical significance across models: `FTR_ratio_sAPPb_sAPPa`, `FCR_ratio_sAPPb_sAPPa`, and `dlt_ratio_sAPPb_Ab40`.
...
Given the high correlation between `FCR_ratio_sAPPb_sAPPa` and `FTR_ratio_sAPPb_sAPPa`, we'll focus on interpreting the latter:

- FTR_ratio_sAPPb_sAPPa:

The estimated coefficients for `FTR_ratio_sAPPb_sAPPa` are consistently **negative**. Using the first table (Model 1) as an example, a one-unit increase in `FTR_ratio_sAPPb_sAPPa` **decreases** the odds of `Amyloid_Status` being 1 by a factor of $e^{-6.507} \approx 0.0015$. This suggests that higher levels of `FTR_ratio_sAPPb_sAPPa` are associated with a lower likelihood of AD[+]. 

However, bearing in mind that the actual range for `FTR_ratio_sAPPb_sAPPa` is 0.721 to 1.109, such a one-unit increase is theoretical. In a realistic increment of 0.1 units, the odds would decrease by \(e^{-0.6507}\approx 0.5\) times.

- dlt_ratio_sAPPb_Ab40:

Similarly, for `dlt_ratio_sAPPb_Ab40`, the estimated coefficients are **negative**. As per the second table (Model 9), a 0.1 unit rise in `dlt_ratio_sAPPb_Ab40` **decreases** the odds of `Amyloid_Status` being 1 by a factor of $e^{-3.22*0.1} \approx 0.725$. Again, this indicates that increased levels of `dlt_ratio_sAPPb_Ab40` are associated with a reduced probability of AD[+].

\
In summary, all the three variables, `FTR_ratio_sAPPb_sAPPa`, `FCR_ratio_sAPPb_sAPPa`, and `dlt_ratio_sAPPb_Ab40`, are indicative of the AD status. Higher values of these variables reduce the likelihood of an individual being AD[+], suggesting their potential use as protective markers against AD.
...
To examine the robustness, we used the default multiple imputation method in addition to the PMM method. Remarkably, **all 14 model** configurations continued to produce significant results under this imputation method, with very similar estimations. For a detailed view of the results, refer to Appendix-Significant Multivariate Logistics Models (Default).
\
