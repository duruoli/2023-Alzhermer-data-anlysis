...
\begin{tabular}{@{}ll}
  ...
  Subject: & Visualization Results of Sheet2
\end{tabular}
...
```
### AD[+] visualization: sAPPb var ~ APOE4_status
...
### AD[+] visualization: continuous control var ~ APOE4_Status
```{r}
...
### AD[+] visualization: categorical control var ~ APOE4_Status
```{r}
...
### AD[+] visualization: control var ~ APOE dose
```{r}
...
## APOE4 Subsets: Compare AD[+] vs AD[-]
...
plot_histograms_with_kdes <- function(var_name, df, title) {
  # First, we map the ApoE_Status to a factor with levels E4- and E4+ for consistent labeling
  df$Amyloid_Status <- factor(df$Amyloid_Status, levels = c("0", "1"), labels = c("AD-", "AD+"))
  
  p <- ggplot(df, aes_string(x = var_name)) +
    geom_histogram(aes(y = ..density.., fill = Amyloid_Status), alpha = 0.5, position = "identity", bins = 30, color = "white") +
    geom_density(aes(color = Amyloid_Status), size = 1.5) +
    scale_fill_manual(values = c("AD-" = "#006400", "AD+" = "red"), labels = c("AD-", "AD+")) +
    scale_color_manual(values = c("AD-" = "#006400", "AD+" = "red"), labels = c("AD-", "AD+")) +
    labs(x = var_name, title = paste(title, "(", var_name, ")"), fill = "Amyloid Status", color = "Amyloid Status") +
    theme_minimal() +
    theme(legend.position = "right",  # Position the legend on the right
          legend.background = element_blank(),
          legend.box.background = element_blank())
...
### E4[+] visualization: sAPPb var ~ Amyloid_Status
```{r}

# Loop through each variable in idp_names to create and display plots
for (var_name in idp_names) {
  ## Overlapped
  p <- plot_histograms_with_kdes(var_name, df=df_E4, title="E4[+]: AD+ vs AD-")
  print(p)  # If running in an interactive R session, this will display the plot.
  
  ## Separated KDE + Histogram
  p1 <- plot_histograms(var_name, df=df_E4, title="E4[+]: AD+ vs AD-")
  p2 <- plot_kdes(var_name, df=df_E4, title="E4[+]: AD+ vs AD-")
  # Arrange plots side by side for each variable
  grid.arrange(p1, p2, ncol=2)
...
### Entire vs E4[+] vs E4[-] visualization: sAPPb var ~ Amyloid_Status
```{r}
